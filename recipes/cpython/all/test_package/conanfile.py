...
class CmakePython3Abi(object):
    ...
    @property
    def suffix(self):
        return f'{"d" if self.debug else ""}{"m" if self.pymalloc else ""}{"u" if self.unicode else ""}'
...
class TestPackageConan(ConanFile):
    ...
    def _py_version(self):
        return re.match(r"^([0-9.]+)", self.deps_cpp_info["cpython"].version)[1]
    ...
    def _cmake_try_FindPythonX(self):
        return (
            ...
            or self.settings.build_type != "Debug"
        )
    ...
    def build(self):
        if not tools.cross_building(self, skip_x64_x86=True):
            command = f'{self.deps_user_info["cpython"].python} --version'
            buffer = StringIO()
        ...
        cmake.definitions[f"Python{py_major}_EXECUTABLE"] = self.deps_user_info[
            "cpython"
        ].python
        cmake.definitions[f"Python{py_major}_ROOT_DIR"] = self.deps_cpp_info[
            "cpython"
        ].rootpath
        cmake.definitions[f"Python{py_major}_USE_STATIC_LIBS"] = not self.options[
            "cpython"
        ].shared
        cmake.definitions[f"Python{py_major}_FIND_FRAMEWORK"] = "NEVER"
        cmake.definitions[f"Python{py_major}_FIND_REGISTRY"] = "NEVER"
        cmake.definitions[f"Python{py_major}_FIND_IMPLEMENTATIONS"] = "CPython"
        cmake.definitions[f"Python{py_major}_FIND_STRATEGY"] = "LOCATION"

        if self.settings.compiler != "Visual Studio" and tools.Version(
            self._py_version
        ) < tools.Version("3.8"):
            cmake.definitions[f"Python{py_major}_FIND_ABI"] = self._cmake_abi.cmake_arg
    ...
    def _test_module(self, module, should_work):
        try:
            self.run(
                f'{self.deps_user_info["cpython"].python} {self.source_folder}/test_package.py -b {self.build_folder} -t {module} ',
                run_environment=True,
            )
            works = True
    ...
    def test(self):
        if tools.cross_building(self, skip_x64_x86=True):
            return
        self.run(
            f"""{self.deps_user_info["cpython"].python} -c \"print('hello world')\"""",
            run_environment=True,
        )

        buffer = StringIO()
        self.run(
            f"""{self.deps_user_info["cpython"].python} -c \"import sys; print('.'.join(str(s) for s in sys.version_info[:3]))\"""",
            run_environment=True,
            output=buffer,
        )
